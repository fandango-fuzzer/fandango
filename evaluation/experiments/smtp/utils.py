...
def _write_coverage_log(file, time_start, coverage_log: list[tuple[float, dict[NonTerminal, tuple[int, int]]]]):
        ...
        for timestamp, coverage_dict in coverage_log:
            if symbol_sorted is None:
                ...
                for symbol in symbol_sorted:
                    f.write(f",covered_{str(symbol)},total_{str(symbol)},percent_{str(symbol)}")
                f.write("\n")
...
def write_coverage_log(fandango, output_folder_name, id_nr, time_start):
    _write_coverage_log(f"{output_folder_name}/run_{id_nr}_grammar_coverage.csv", time_start, fandango.coverage_log)
    _write_coverage_log(f"{output_folder_name}/run_{id_nr}_grammar_coverage_overlap.csv", time_start, fandango.coverage_log_overlap)
