    ...
    for folder, spec in protocols:
        ...
        try:
            ...
            with open(file_path, "w") as f:
                ...
                for session in fandango.packet_selector._all_derivation_trees():
                    for msg in session.protocol_msgs():
                        if msg.sender in ignore_parties:
                            continue
                        msgs_by_sender[msg.sender] = (
                            msgs_by_sender.get(msg.sender, 0) + 1
                        )
                        overall_msgs += 1
                f.write(
                    f"Nr trees generated: {len(fandango.packet_selector._all_derivation_trees())}\n"
                )
                f.write(f"Nr messages exchanged: {overall_msgs}\n")
                ...
                f.write(
                    f"Netto time elapsed: {time.time() - (start_time + fandango._time_in_measurements):.2f}s\n"
                )
                f.write(
                    f"Grammar coverage: {(fandango.packet_selector.coverage_percent(alt_cache=True)*100):.2f}\n"
                )
