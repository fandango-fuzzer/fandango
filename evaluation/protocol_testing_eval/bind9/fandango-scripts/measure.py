"""
Measurement runs for the experiments.
  (no --experiment)         run Fandango normally (guidance on, stop at full
                            grammar coverage).
  --experiment throughput   run Fandango normally, but switches guidance off
                            after reaching full coverage. Continues till --duration
                            has been reached.
  --experiment coverage     runs coverage experiment, with guidance on or off
                            (--guidance arg), logs coverage every --interval steps
                            stop at 100% coverage or if --duration has been reached,
                            or once start-symbol coverage stops rising for --plateau-timeout
"""
import argparse
import signal
import time
...
# Filter messages that have been generated by helper parties.
...
def run(fandango, args, max_generations=None):
    ...
    try:
        ...
        if args.experiment == "throughput":
            # in-progress run
            _tally_messages(fandango, fandango._protocol_tree, tally)
