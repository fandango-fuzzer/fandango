...
<handshake_with_retransmit> ::= \
    <TimerControl:rekey_timeout_start> <TimerControl:rekey_attempt_start> <area_msg_initiator> <await_handshake_response>

<await_handshake_response> ::= \
    <area_msg_responder> <post_response> \
  | <TimerEvent:rekey_timeout_expired> <TimerControl:rekey_timeout_start> <area_msg_initiator> <await_handshake_response> \
...
where all(int(<env>..<keepalive_start>..<timer_timeout>) == KEEPALIVE_SECONDS for <env> in *<keepalive_start>)
where all(int(<env>..<keepalive_start>..<timer_id>)      == TIMER_ID_KEEPALIVE  for <env> in *<keepalive_start>)
where all(int(<env>..<keepalive_cancel>..<timer_id>)     == TIMER_ID_KEEPALIVE  for <env> in *<keepalive_cancel>)
where all(int(<env>..<keepalive_expired>..<timer_id>)    == TIMER_ID_KEEPALIVE  for <env> in *<keepalive_expired>)
...
where all(int(<env>..<rekey_after_time_start>..<timer_timeout>) == REKEY_AFTER_TIME_SECONDS for <env> in *<rekey_after_time_start>)
where all(int(<env>..<rekey_after_time_start>..<timer_id>)      == TIMER_ID_REKEY_AFTER_TIME for <env> in *<rekey_after_time_start>)
where all(int(<env>..<rekey_after_time_cancel>..<timer_id>)     == TIMER_ID_REKEY_AFTER_TIME for <env> in *<rekey_after_time_cancel>)
where all(int(<env>..<rekey_after_time_expired>..<timer_id>)    == TIMER_ID_REKEY_AFTER_TIME for <env> in *<rekey_after_time_expired>)
...
where all(int(<env>..<reject_after_time_start>..<timer_timeout>) == REJECT_AFTER_TIME_SECONDS for <env> in *<reject_after_time_start>)
where all(int(<env>..<reject_after_time_start>..<timer_id>)      == TIMER_ID_REJECT_AFTER_TIME for <env> in *<reject_after_time_start>)
where all(int(<env>..<reject_after_time_cancel>..<timer_id>)     == TIMER_ID_REJECT_AFTER_TIME for <env> in *<reject_after_time_cancel>)
where all(int(<env>..<reject_after_time_expired>..<timer_id>)    == TIMER_ID_REJECT_AFTER_TIME for <env> in *<reject_after_time_expired>)
...
where all(int(<env>..<rekey_timeout_start>..<timer_timeout>) == REKEY_TIMEOUT_SECONDS for <env> in *<rekey_timeout_start>)
where all(int(<env>..<rekey_timeout_start>..<timer_id>)      == TIMER_ID_REKEY_TIMEOUT for <env> in *<rekey_timeout_start>)
where all(int(<env>..<rekey_timeout_cancel>..<timer_id>)     == TIMER_ID_REKEY_TIMEOUT for <env> in *<rekey_timeout_cancel>)
where all(int(<env>..<rekey_timeout_expired>..<timer_id>)    == TIMER_ID_REKEY_TIMEOUT for <env> in *<rekey_timeout_expired>)
...
where all(int(<env>..<rekey_attempt_start>..<timer_timeout>) == REKEY_ATTEMPT_TIME_SECONDS for <env> in *<rekey_attempt_start>)
where all(int(<env>..<rekey_attempt_start>..<timer_id>)      == TIMER_ID_REKEY_ATTEMPT for <env> in *<rekey_attempt_start>)
where all(int(<env>..<rekey_attempt_cancel>..<timer_id>)     == TIMER_ID_REKEY_ATTEMPT for <env> in *<rekey_attempt_cancel>)
where all(int(<env>..<rekey_attempt_expired>..<timer_id>)    == TIMER_ID_REKEY_ATTEMPT for <env> in *<rekey_attempt_expired>)
...
where all(int(<env>..<new_handshake_start>..<timer_timeout>) == NEW_HANDSHAKE_TIMEOUT_SECONDS for <env> in *<new_handshake_start>)
where all(int(<env>..<new_handshake_start>..<timer_id>)      == TIMER_ID_NEW_HANDSHAKE for <env> in *<new_handshake_start>)
where all(int(<env>..<new_handshake_cancel>..<timer_id>)     == TIMER_ID_NEW_HANDSHAKE for <env> in *<new_handshake_cancel>)
where all(int(<env>..<new_handshake_expired>..<timer_id>)    == TIMER_ID_NEW_HANDSHAKE for <env> in *<new_handshake_expired>)
