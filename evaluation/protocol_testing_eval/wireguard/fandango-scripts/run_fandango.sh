...
start_boringtun() {
  ...
  if [ "$TUN_OK" -ne 1 ]; then
    echo "wg0 interface did not appear." >&2
    cat "${PROFRAW_DIR}/boringtun.log" >&2 || true
    return 1
  fi
  echo "wg0 is up."
...
configure_wg() {
  # Retry until 'wg show' ensures that the listen port and the peer are configured.
  local i
  for i in $(seq 1 60); do
    ...
    if wg show "$WG_IF" 2>/dev/null | grep -q "listening port: ${WG_PORT}" \
      ...
      # Interface address + link up to make the container answer ping 10.13.13.1.
...
for attempt in 1 2 3; do
  ...
  ip link del "$WG_IF" 2>/dev/null || true   # drop a old wg0
  BORINGTUN_PID=""
  sleep 1
done
if [ "$BOOT_OK" -ne 1 ]; then
  echo "boringtun never came up cleanly after retries." >&2
fi
