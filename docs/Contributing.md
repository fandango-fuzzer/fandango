...
- `uv.lock` is the lockfile generated by `uv` and captures the actual dependency set used in this project (i.e., source of truth).
- `pylock.toml` is derived from `uv.lock` to support a wider range of ecosystem tools.

After updating dependencies in `pyproject.toml`, make sure to regenerate `uv.lock` with `uv lock` and update `pylock.toml` accordingly.
