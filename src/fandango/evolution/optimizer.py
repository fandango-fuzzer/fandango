...
def genetic_algorithm_distribution(constraints, num_values, ngen, verbose=False):
    ...
    def fitness_uniform(individual):
        return (
            -np.sum(
                np.abs(
                    np.histogram(individual, bins=30, range=(min_val, max_val))[0]
                    - num_values / 30
                )
            ),
        )
    ...
    def fitness_normal(individual):
        target_distribution = np.clip(
            norm.rvs((min_val + max_val) / 2, (max_val - min_val) / 6, size=num_values),
            min_val,
            max_val,
        ).astype(int)
        return (
            -np.sum(
                np.abs(
                    np.histogram(individual, bins=30, range=(min_val, max_val))[0]
                    - np.histogram(
                        target_distribution, bins=30, range=(min_val, max_val)
                    )[0]
                )
            ),
        )
    ...
    def fitness_inverse(individual):
        return (
            -np.sum(
                np.abs(
                    np.histogram(
                        sorted(individual, reverse=True),
                        bins=30,
    ...
    def feasible(individual):
        ...
        if min_diff is not None:
            if any(
                abs(individual[i] - individual[i + 1]) < min_diff
    ...
    def distance(individual):
        ...
        if min_diff is not None:
            penalty += sum(
                max(0, min_diff - abs(individual[i] - individual[i + 1]))
                for i in range(len(individual) - 1)
            )
    ...
    algorithms.eaSimple(
        population, toolbox, cxpb=0.5, mutpb=0.2, ngen=ngen, verbose=verbose
    )
...
def fill_test_suite(test_suite, constraints, ngen, verbose, plot=False):
    ...
    if plot:
        for attribute, values in generated_values.items():
            plot_histogram(
                values, f"Distribution of {attribute}", attribute, "Frequency"
            )
