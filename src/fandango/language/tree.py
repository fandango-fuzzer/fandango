    ...
    def _replace(self, grammar: "Grammar", tree_to_replace, new_subtree):
        ...
        if regen_children:
            ...
            while current_parent is not None:
                ...
                if current in current_parent.children and len(current_parent.generator_params) != 0:
                    is_generator_generated = True
                    break
                elif current in current_parent.generator_params:
                    break
                current = current_parent
                current_parent = current_parent.parent
            # Trees generated by generators don't contain children generated with other generators.
            # Break infinite loop if nonterminal already part of current path. (Prevent nesting of the same generator)
            if is_generator_generated or new_tree.symbol in set(new_tree.get_path()[:-1]):
                new_tree.generator_params = []
            else:
